...
////////////////////////////////////////////////////////////////////////////////////
// Actualizado	: Jesus David Peñuelas
// Fecha		: 25-MAR-08
// Actividad	: Agregar el metodo SetXML para especificar la ruta del documento XML
//				  que se va a cargar.
////////////////////////////////////////////////////////////////////////////////////
// Actualizado	: Jesus David Peñuelas
// Fecha		: 18-ABRIL-08
// Actividad	: Optimizacion de la carga en memoria del documento XML
//				  
...
bool CMarkup::SetXML( LPCTSTR szRutaXML )
{
	CString sXML;	
	CString csNotes, sEtiqueta, sEtiquetaAtrib;	
	CFile file;
	int nFileLen = 0;		
	bool bRespuesta = false;	
	
	if( access( szRutaXML, 0 ) == 0 )
	{
		if ( file.Open( szRutaXML, CFile::modeRead ) )
		{				
			...
			if ( pBuffer[0] == 0xff && pBuffer[1] == 0xfe )
			{
				...
				sXML = (LPCWSTR)(&pBuffer[2]);
				m_csError.Format( _T( "Archivo inicia con FFFF, se asume que es formato wide char.") );
			}
			else
			{
			...
			if ( sXML.GetLength() >= nFileLen / 2 - 20 )
			{		
				bRespuesta = SetDoc( sXML );
			}
			else
			{
				m_csError.Format( _T( "Error al convertir archivo a string (Posiblemente contenga datos binarios)" ) );
			}		
		}
		else
		{
			m_csError.Format( _T( "%d: %s" ), errno, strerror( errno ) );
		}		
	}
	else
	{
		m_csError.Format( _T( "No existe el documento %s" ), szRutaXML );
	}	
...
bool CMarkup::x_AddElem( LPCTSTR szName, LPCTSTR szValue, bool bInsert, bool bAddChild )
{
	...
	int nLenValue = (int)( szValue? _tcslen(szValue) : 0 );
	if ( ! nLenValue )
	{
		// <NAME/> empty element
		csInsert = _T("<");
