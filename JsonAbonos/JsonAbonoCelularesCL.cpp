...
JsonAbonoCelularesCL::JsonAbonoCelularesCL(void)
{
}
...
void JsonAbonoCelularesCL::agregarAbono(long cliente, long importe, int meses, int tienda, long minimo, int caja,
...
std::string JsonAbonoCelularesCL::getJson() const {
	CString dato = "";
    ...
    for (size_t i = 0; i < abonos.size(); ++i) {
		...
		dato.Format("%ld", abonos[i].minimo);
		json += "\"minimo\": \"" + dato + "\"";
		json += "}";
        if (i != abonos.size() - 1) {
            json += ",";
...
bool JsonAbonoCelularesCL::vacio() {
	if(abonos.size() > 0){
		return false;
	}
	return true;
...
void JsonAbonoCelularesCL::limpiar() {
    abonos.clear();
}

void JsonAbonoCelularesCL::eliminarAbono(CString poliza){
	CString polizaAbono = "";
	int tamanio = 0;
	int eliminar = -1;
	std::vector<Abono> abonosCapturados;
	if(!this->vacio())
	{
		tamanio = abonos.size();
