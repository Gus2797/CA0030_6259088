...
int CA0030(char *cInput1, char *cInput2)
{
    ...
    if (!checarLink(sParametros.iLink)) //TODO: 01/09/2025, AGREGADO POR RMS, FLUJO DE guardarCertificacionAbonos
    {
        ...
        else
        {
            ...
            AfxMessageBox("Error al abrir conexi�n BD TIENDA.####");
        }
        if (iFlagConexion == 1)
        {
            if (dlg.iSistema != SISTEMA_ROPA && dlg.iSistema != SISTEMA_MUEBLES)
            {
                ...
                else
                {
                    while (bSalir && dlg.iFlagCorte != 1)
                    {
						...
						{
							// SI EST� PRENDIDO EL FLAG MUESTRA EL MENU
							...
							{
								// SI NO EST� PRENDIDO SE MUESTRA DIRECTAMENTE LA OPCION F1
								dlg.iOpcionSeleccionadaMenuInicial = F1;
							}
						}
						else
						{
							// SI NO SE HACE LA CONSULTA NO SE MUESTRA EL MEN�
							dlg.iOpcionSeleccionadaMenuInicial = F1;
						}
                        ...
                        case F1:
                        {
                            if (dlg.iSistema == SISTEMA_CAJAS)
                            {
                                ...
                                {
                                    CString sTexto;
                                    sTexto.Format("EL EMPLEADO %ld NO TIENE DERECHOS EN CAJAS, POR LO CUAL NO SERA TOMADO EN CUENTA\nEN EL REPORTE DE LA EFICIENCIA DE LAS CAJERAS, USE LA OPCI�N DE ALTA DE CLAVES", dlg.m_grid.lEmpleado);
